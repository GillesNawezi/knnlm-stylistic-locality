...
class WeightedDist(torch.nn.Module):
    ...
    def forward(self, context_vec, dist, pkg_l, idx_mask):
        ...
        probs = utils.log_softmax(locality_feat[0] * (params[:, 0][:, None] * dist) +
                                  locality_feat[1] * (params[:, 1][:, None] * dist + params[:, 2][:, None]), dim=-1)
        ...
        # probs = utils.log_softmax(dist, dim=-1)6
...
print(f"pkg_locality: {pkg_locality.shape}")
print(f"index_masks: {index_masks.shape}")
print(f"lm_probs: {lm_probs.shape}")
...
print(f"test_context_vecs: {test_context_vecs.shape}")
print(f"test_dists: {test_dists.shape}")
print(f"test_pkg_locality: {test_pkg_locality.shape}")
print(f"test_index_masks: {test_index_masks.shape}")
print(f"test_lm_probs: {test_lm_probs.shape}")
...
valid_dataset = TensorDataset(context_vecs, 
                              dists, 
                              pkg_locality, 
                              #proj_locality, 
                              index_masks)

test_dataset = TensorDataset(test_context_vecs, 
                             test_dists, 
                             test_pkg_locality, 
                             #test_proj_locality, 
                             test_index_masks)
