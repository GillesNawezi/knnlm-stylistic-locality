...
class WeightedDist(torch.nn.Module):
    ...
    def forward(self, context_vec, dist, pkg_l, proj_l, cat_l, idx_mask):
        """
        """
        ...
        probs = utils.log_softmax(locality_feat[0] * (params[:, 0][:, None] * dist) +
                                  ...
                                  locality_feat[3] * (params[:, 5][:, None] * dist + params[:, 6][:, None]) +
                                  locality_feat[4] * (params[:, 7][:, None] * dist + params[:, 8][:, None]) +
                                  locality_feat[5] * (params[:, 9][:, None] * dist + params[:, 10][:, None]) +
                                  locality_feat[6] * (params[:, 11][:, None] * dist + params[:, 12][:, None]), dim=-1)
...
cat_locality = np.load(f'saved_tensors/{dataset}/valid_cat_locality_cache.npy').reshape(-1, num_retrieved)
...
cat_locality = cat_locality[indices]
...
test_cat_locality = np.load(f'saved_tensors/{dataset}/test_cat_locality_cache.npy').reshape(-1, num_retrieved)
...
test_cat_locality = test_cat_locality[test_indices]
...
test_cat_locality = torch.from_numpy(test_cat_locality)
