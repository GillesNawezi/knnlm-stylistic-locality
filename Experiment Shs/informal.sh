...
## preprocess
TEXT=examples/language_model/informal_dataset
python preprocess.py \
    --only-source \
    ...
    --destdir data-bin/informal_dataset \
    --workers 20
...
CUDA_VISIBLE_DEVICES=1 python train.py --task language_modeling \
    data-bin/informal_dataset \
    --save-dir checkpoints/informal_dataset \
    --arch transformer_lm_wiki103 \
    --max-epoch 300 --max-lr 1.0 --t-mult 2 --lr-period-updates 270000 --lr-scheduler cosine --lr-shrink 0.75 \
    --warmup-updates 1255577 --warmup-init-lr 1e-07 --min-lr 1e-09 --optimizer nag --lr 0.0001 --clip-norm 0.1 \
...
## eval lm, valid
python eval_lm.py data-bin/informal_dataset \
    --path checkpoints/informal_dataset/checkpoint_best.pt \
...
python eval_lm.py data-bin/informal_dataset \
    ...
    --dstore-mmap checkpoints/informal_dataset/valid_dstore --knn-keytype 'last_ffn_input' \
    --dstore-size 255577  --model-overrides "{'knn_keytype': 'last_ffn_input'}" \
...
python eval_lm.py data-bin/informal_dataset \
    ...
    --tokens-per-sample 3072 \
    --dstore-mmap checkpoints/informal_dataset/test_dstore --knn-keytype 'last_ffn_input' \
    --dstore-size 255577  --model-overrides "{'knn_keytype': 'last_ffn_input'}" \
    --save-knnlm-dstore --fp16
...
python build_dstore.py \
    --dstore_mmap checkpoints/informal_dataset/valid_dstore \
    --dstore_size 255577 \
    --faiss_index checkpoints/informal_dataset/valid_knn.index \
    --num_keys_to_add_at_a_time 500000 \
    --starting_point 0 --dimension 1024

## build test index
python build_dstore.py \
    --dstore_mmap checkpoints/informal_dataset/test_dstore \
    --dstore_size 255577 \
    --faiss_index checkpoints/informal_dataset/test_knn.index \
    --num_keys_to_add_at_a_time 500000 \
    --starting_point 0 --dimension 1024
...
python eval_lm.py data-bin/informal_dataset \
    ...
    --softmax-batch 1024 \
    --gen-subset valid \
    --dstore-filename checkpoints/informal_dataset/valid_dstore \
    --indexfile checkpoints/informal_dataset/valid_knn.index  \
    --model-overrides "{'knn_keytype': 'last_ffn_input'}" \
    --k 1024 --lmbda 0.25 --dstore-size 255577 --knn-keytype last_ffn_input \
...
python eval_lm.py data-bin/informal_dataset \
    ...
    --softmax-batch 1024 \
    --gen-subset test \
    --dstore-filename checkpoints/informal_dataset/test_dstore \
    --indexfile checkpoints/informal_dataset/test_knn.index  \
    --model-overrides "{'knn_keytype': 'last_ffn_input'}" \
    --k 1024 --lmbda 0.25 --dstore-size 255577 --knn-keytype last_ffn_input \
    --probe 32 --knnlm  --fp16 --knn-sim-func "do_not_recomp_l2" --no-load-keys --move-dstore-to-mem \
...
--path checkpoints/informal_dataset/wt103_checkpoint_best.pt \
--beam 5

#Wiki Model Fine Tuned with Style Data
fairseq-interactive data-bin/informal_dataset \
--task language_modeling \
--path checkpoints/informal_dataset/checkpoint_best.pt \
--beam 5
...
#%%
fairseq-generate data-bin/informal_dataset \
    --task language_modeling \
    --path checkpoints/informal_dataset/checkpoint_best.pt \
    --gen-subset valid
    --beam 5 
...
python generate.py data-bin/informal_dataset \
  --path checkpoints/informal_dataset/checkpoint_best.pt \
  --beam 5 \
  --batch-size 16 \
  --nbest 5
