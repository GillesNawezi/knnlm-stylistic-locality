...
## preprocess
TEXT=examples/language_model/polite_dataset
python preprocess.py \
    --only-source \
    ...
    --destdir data-bin/polite_dataset \
    --workers 20
...
CUDA_VISIBLE_DEVICES=3 python train.py --task language_modeling \
    data-bin/polite_dataset \
    --save-dir checkpoints/polite_dataset \
...
## eval lm, valid
python eval_lm.py data-bin/polite_dataset \
    --path checkpoints/polite_dataset/checkpoint_best.pt \
...
python eval_lm.py data-bin/polite_dataset \
    ...
    --dstore-mmap checkpoints/polite_dataset/valid_dstore --knn-keytype 'last_ffn_input' \
...
python eval_lm.py data-bin/polite_dataset \
    ...
    --tokens-per-sample 3072 \
    --dstore-mmap checkpoints/polite_dataset/test_dstore --knn-keytype 'last_ffn_input' \
    --dstore-size 6000  --model-overrides "{'knn_keytype': 'last_ffn_input'}" \
    --save-knnlm-dstore --fp16
...
python build_dstore.py \
    --dstore_mmap checkpoints/polite_dataset/valid_dstore \
    --dstore_size 6000 \
    --faiss_index checkpoints/polite_dataset/valid_knn.index \
    --num_keys_to_add_at_a_time 500000 \
    --starting_point 0 --dimension 1024

## build test index
python build_dstore.py \
    --dstore_mmap checkpoints/polite_dataset/test_dstore \
    --dstore_size 6000 \
    --faiss_index checkpoints/polite_dataset/test_knn.index \
    --num_keys_to_add_at_a_time 500000 \
    --starting_point 0 --dimension 1024
...
python eval_lm.py data-bin/polite_dataset \
    ...
    --softmax-batch 1024 \
    --gen-subset valid \
    --dstore-filename checkpoints/polite_dataset/valid_dstore \
    --indexfile checkpoints/polite_dataset/valid_knn.index  \
...
python eval_lm.py data-bin/polite_dataset \
    ...
    --softmax-batch 1024 \
    --gen-subset test \
    --dstore-filename checkpoints/polite_dataset/test_dstore \
    --indexfile checkpoints/polite_dataset/test_knn.index  \
    --model-overrides "{'knn_keytype': 'last_ffn_input'}" \
    --k 1024 --lmbda 0.25 --dstore-size 6000 --knn-keytype last_ffn_input \
    --probe 32 --knnlm  --fp16 --knn-sim-func "do_not_recomp_l2" --no-load-keys --move-dstore-to-mem \
...
--path checkpoints/polite_dataset/wt103_checkpoint_best.pt \
--beam 5

#Wiki Model Fine Tuned with Style Data
fairseq-interactive data-bin/polite_dataset \
--task language_modeling \
--path checkpoints/polite_dataset/checkpoint_best.pt \
--beam 5
...
#%%
fairseq-generate data-bin/polite_dataset \
    --task language_modeling \
    --path checkpoints/polite_dataset/checkpoint_best.pt \
    --gen-subset valid
    --beam 5 
...
python generate.py data-bin/polite_dataset \
  --path checkpoints/polite_dataset/checkpoint_best.pt \
  --beam 5 \
  --batch-size 16 \
  --nbest 5
